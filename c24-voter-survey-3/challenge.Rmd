...
__q1__ Perform your own exploratory data analysis of the voters who voted for Obama in 2012 and Trump in 2016. In addition to the data `file_voter_survey`, you can join in data from the complete VOTER Survey dataset from Part 1. What conclusions can you draw?
...
vs %>%
  ...
  summarise_at(
    ...
    funs(weighted.mean(., wt = weight), sd),
    na.rm = TRUE
  ) %>%  
  gather(key = "var", val = "value", economic_weighted.mean:social_weighted.mean) %>%
  ggplot(aes(x = var, y = value)) +
  geom_point() +
  geom_hline(yintercept = 0) +
  ylim(-1, 1) +
