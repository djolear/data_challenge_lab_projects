...
__q1__ The 2016 VOTER Survey dataset is available at [this site](https://www.voterstudygroup.org/reports/2016-elections/data). Download the documentation (.doc file) and the data in CSV format and save it in a directory other than a GitHub repo. Read the data into the tibble `voter_survey`.
...
__q2__ Each of the derived variables is based upon the responses to more than one question. Now that the responses to the questions have been recoded so that they are all numbers between -1 and 1, the derived variables are calculated by taking the mean. For example, the `inequality` variable is the mean of `inequality_1`, `inequality_2`, and `inequality_3`. In this way, the 37 variables `rigged_1`, `rigged_2`, ..., `government_2` become the 12 variables `rigged`, `safety`, ..., `government`. Two additional composite variables were created as follows:
...
__q3__ Finally, we need to combine the derived variables with information about the voter and how they voted. Using `voter_survey` and `q2`, create a new tibble `q3` with the following variables:
